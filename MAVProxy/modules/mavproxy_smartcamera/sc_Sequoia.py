...
class SmartCameraSequoia:
    ...
    def get_latest_image(self):
        ...
        if self.debug: print (imgfilename)
    ...
    def take_picture(self):
        # setup image capture
        with self.camera.session():
            if self.debug: print("%s Taking Picture" % self.config_group)
            success_flag = None
            # with self.camera.session():
            response = self.camera.initiate_capture()
            if self.debug: print("image counts: %d / %d" % (self.num_stored_images, self.get_num_stored_images(self.storage_id)))
            if (response.ResponseCode == 'OK'):
                ...
                if (new_num_images == self.num_stored_images):
                    if self.debug: print("ERROR: Got an OK from camera trigger but found no new image")
                    success_flag = False
                else:
                    print("New image found in camera memory")
    ...
    def main(self):
        ...
        while True:
            ...
            if self.take_picture():
                ...
                if self.debug: print "no new image available"
